...
export type DashboardDeps = {
  ...
  tasks: TaskStore
  runs: AgentRunStore
  /** Canonical task source override. File-managed callers use next.yml instead of SQLite here. */
...
export function getProjectDashboard(deps: DashboardDeps, projectId: string): ProjectDashboard {
  ...
  const all = deps.listTasks?.(projectId) ?? deps.tasks.listByProject(projectId)

...
│  - AgentConnector (Claude/Codex/OpenCode)     │
│  - SourceNormalizer                           │
│  - ConflictManager                            │
...
- **Phase 0 — Common Core 먼저.** 화면도 LLM Wiki도 아닌 다음 6개를 먼저 만든다:
  `ProjectRegistry`, `ObsidianVaultAdapter`, `AgentConnector interface`,
...
packages/
  ...
  connectors/              # Claude / Codex / OpenCode connector
  llm-wiki/                # LLM Wiki pipeline + AgentRunner
...
각 connector는 자기 포맷만 책임지고, 결과는 반드시 `NormalizedSession`으로 정규화한다.
...
## 7. LLM Wiki 엔진 (확정: 로컬 CLI 헤드리스 재사용)
...
- **트리거 = on-demand("갱신" 클릭).** 매 ingest마다 느린 CLI를 자동 호출하지 않는다.
- 사용할 CLI는 **프로젝트별/기본값으로 설정 가능**.
...
- **Claude connector 우선 완성**, Codex·OpenCode는 인터페이스 + 최소 구현
...
- 충돌 문서 생성
- LLM Wiki: candidate/proposal까지 자동, canonical 반영은 승인 (질문 2 = A/B/C 자동, D/E 승인)
...
- IDE / 터미널 / 파일트리 패널 (NexusCode식 확장, P2)
...
- **Connector**: 이 머신의 실제 샘플 로그를 fixture로 박제한 **골든 테스트**
...
| 세 agent 포맷 변경 | connector 어댑터 격리 + 골든 fixture로 회귀 탐지 |

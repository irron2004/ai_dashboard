...
- **Plan 6 — Electron shell + Terminal + PM Control Tower UI** (`@apc/app-services` + `apps/desktop`): integration services (`IngestService`, `RunService`, `CurrentPromotionService`) composing Plans 1–5, typed IPC BFF, React PM Control Tower (PmHome / ModelPicker / ReviewActions / HarnessPanel), and `node-pty`+`xterm.js` Agent Work Execution Panel. → `2026-06-01-agent-project-console-electron-ui.md`. Parts A–C are TDD-able headless; Part D (Electron + native `node-pty`) is verified on a dev machine (this WSL2 box has no compiler/display).
